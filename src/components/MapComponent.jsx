        ...
        <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-20 w-11/12 md:w-96 max-w-md">
          <div className="bg-white/95 backdrop-blur-md rounded-2xl shadow-2xl p-6 border-2 border-white/50 animate-in fade-in zoom-in duration-200">
            ...
            <div className="flex items-start gap-3 mb-4">
              <div className={`w-12 h-12 rounded-full flex items-center justify-center ${selectedEvent.hasFood ? 'bg-yellow-400 text-black' : 'bg-indigo-600 text-white'}`}>
                {selectedEvent.hasFood ? '🍕' : '⚡'}
              </div>
              <div className="flex-1">
                <h3 className="text-xl font-bold text-slate-800">{selectedEvent.title}</h3>
                <p className="text-sm text-slate-500">{selectedEvent.category}</p>
              </div>
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-2 text-slate-700">
                ...
                <span className="font-medium">{locationCoords[selectedEvent.locationId]?.name || selectedEvent.locationId.replace(/_/g, ' ')}</span>
              </div>

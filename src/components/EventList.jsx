...
const EventList = ({ events = [], selected = 'upcoming', setSelected = () => {}, mapping = {} }) => {
  ...
  return (
    <>
      ...
      <div className="hidden sm:block fixed left-4 top-20 bottom-4 w-80 z-20">
        <div className="h-full backdrop-blur supports-[backdrop-filter]:bg-white/60 bg-white/80 rounded-2xl shadow-xl border border-slate-200/50 overflow-hidden flex flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4">
            {events.length === 0 ? (
              <p className="text-slate-500 text-center py-4">No {selected} events yet.</p>
            ) : (
              <ul className="space-y-3">
                {events.map(event => {
                  ...
                  return (
                    <li key={event.id} className="p-3 bg-indigo-50 rounded-lg hover:bg-indigo-100 transition-colors cursor-pointer border border-indigo-100">
                      <div className="flex items-start justify-between">
                        <div className="flex-1">
                          <h3 className="font-bold text-base text-slate-900">{event.title}</h3>
                          <p className="text-xs text-slate-600 mt-1">{event.category} @ {event.locationId.replace(/_/g, ' ')}</p>
                          <p className="text-slate-700 mt-1 text-sm">{event.description}</p>
                          {(event.start_time || event.end_time) && (
                            <div className="mt-2 text-xs text-slate-600 space-y-0.5">
                              {event.start_time && <p>Starts: {formatDateTime(event.start_time)}</p>}
                              {event.end_time && <p>Ends: {formatDateTime(event.end_time)}</p>}
                            </div>
                          )}
                        </div>
                        {event.hasFood && (
      ...
      <div className="sm:hidden">
        ...
        `}>
          <div onClick={() => setIsOpen(!isOpen)} className="cursor-pointer h-12 flex justify-center items-center">
            <ChevronUp className={`transition-transform text-slate-700 ${isOpen ? 'rotate-180' : ''}`} />
          </div>
                ...
                {events.length === 0 ? (
                  ...
                  <ul className="space-y-3">
                    {events.map(event => {
                      ...
                      return (
                        <li key={event.id} className="p-3 bg-indigo-50 rounded-lg hover:bg-indigo-100 transition-colors border border-indigo-100">
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <h3 className="font-bold text-base text-slate-900">{event.title}</h3>
                              <p className="text-xs text-slate-600 mt-1">{event.category} @ {event.locationId.replace(/_/g, ' ')}</p>
